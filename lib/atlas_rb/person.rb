...
module AtlasRb
  ...
  class Person < Resource
    ...
    # @param on_behalf_of [String, nil] acting-as target.
    # @return [Array<AtlasRb::Mash>, nil] one Person per row on the page. Returns
    ...
    # @param nuid [String, nil] acting principal.
    # @param on_behalf_of [String, nil] acting-as target.
    # @return [Array<AtlasRb::Mash>, nil] one Person per resolved NUID (each carries
    ...
    def self.resolve(nuids, nuid: nil, on_behalf_of: nil)
      read_body(
        connection({ nuids: Array(nuids).join(",") }, nuid, on_behalf_of: on_behalf_of).get(ROUTE)
      ) { |body| body["people"].map { |entry| AtlasRb::Mash.new(entry) } }
    end
